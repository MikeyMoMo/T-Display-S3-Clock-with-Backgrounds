...
TFT_eSprite spriteDateWork = TFT_eSprite(&tft);  // Date sprite invisible.
...
/*******************************************************************************************/
{
  ...
  while (file) {
    //    Serial.print("Found LittleFS file: /");
  ...
  //  First, I will drawString the text onto spriteDateWork, then I will push that onto
  //  spriteDate with black invisible then push that onto spriteBG for the final
  //  presentation.  Only thing is to have only one of the setSwapByte be true and the
  //  other one be false so the colors will come out right.
  // Date sprite.
  spriteDate.createSprite(SPR_DATE_WIDTH, SPR_DATE_HEIGHT);
  spriteDate.setSwapBytes(true);  // 1
  // Invisible Work Date sprite.
  spriteDateWork.createSprite(SPR_DATE_WIDTH, SPR_DATE_HEIGHT);
  spriteDate.setSwapBytes(false);  // 2

  // Battery.
  spriteBatt.createSprite(SPR_BATTERY_WIDTH, SPR_BATTERY_HEIGHT);
...
/*******************************************************************************************/
{
  ...
  if (prev_BL_Hour != iHour) {
    prev_BL_Hour = iHour;
    pickHourBrightness();  // This is called by HourDance, too.
  }
  ...
  if (iMinute % PicChgFreq == 0 & iSecond == 0) {
    ...
    BGPic = iTemp ; prev_BGPic = -1; // Force one pass thru file name display.
  }
